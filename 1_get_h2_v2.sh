...
wd=/mnt/sdb/genetics/delme/ldaktest
...
#If output folder doesnt exist yet, just make it
 qsub -t1-22 -l walltime=00:15:00 scripts/2a_identify_related.qsub -d $wd -e errandout/ -o errandout/ -F "-e $ldakf -l ldakmodel -p $power -q $pruning -w $windowkb -r $r2 -n $sname -g $genodir_relatedness -u $useweights -o $outdir_relatedness -x $lisa"
...
##2B: Prepare phenotype and covariates data.
 ...
 phenofile=pgc-ptsd-all_v1.txt #Phenotpye file should be blankspace delimited. It should have columns 1 and 2 be FID and IID. It must have a header. Put phenotype and covariates in here
...
#Note: Manual suggests doing this by cohort to prevent genotyping errors from influencing weights. However,this is impractical because of our number of cohorts and their wildly different sizes, so I don't do it!
 ...
 qsub -t22 -l walltime=00:30:00 scripts/3_ldak_weights.qsub -d $wd -e errandout/ -o errandout/  -F "-e $ldak -g $outdir_mgeno -n $sname -k $weightings_keepflag -o $outdir_weights -x $lisa"
...
#3B: Calculate kinship for each chromosome
 ...
 qsub -t1-22 -l walltime=00:05:00 scripts/3B_ldak_kinship_v2_alt.qsub -d $wd -e errandout/ -o errandout/ -F "-e $ldakf -l $ldakmodel -p $power -n $sname -g $outdir_mgeno -o $kinship_dir -p $power -w $outdir_weights -x $lisa"
...
#Program parameters:
 ...
 qsub -l walltime=00:05:00 scripts/3C_merge_kinships.qsub -d $wd -e errandout/ -o errandout/ -F "-e $ldak -k $kinship_dir -o $combkinship_dir -f kinships -b yes -x $lisa"
...
#Program parameters:
 ...
 qsub -l walltime=00:05:00 scripts/4_h2.qsub -d $wd -e errandout/ -o errandout/ -F "-e $ldak  -p $phenotype -c $covar -g $combkinship_dir -k $keeplist -o $ldak_resultsdir -a $outname_append -b yes -r $prevalence -x $lisa"
...
#Now we can do the actual estimate of h2 snp
 qsub -l walltime=00:05:00 scripts/4_h2.qsub -d $wd -e errandout/ -o errandout/ -F "-e $ldakf  -p $phenotype -c $covar -g $combkinship_dir -k $keeplist -o $ldak_resultsdir -a $outname_append -b no -r $prevalence -x $lisa"
